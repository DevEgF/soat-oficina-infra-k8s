...
Before the first application Helm deployment, an EKS administrator runs the
foundation-owned namespace/RBAC bootstrap with an explicit kubeconfig and cluster
ARN. Its context must be the ARN generated by `aws eks update-kubeconfig`.
The bootstrap intentionally accepts only the approved commercial AWS partition
and `us-east-1` region; a deployment outside that contract requires a reviewed change.

```powershell
pwsh -File scripts/bootstrap-app-namespaces.ps1 -Kubeconfig .tmp/eks-config -ClusterArn arn:aws:eks:us-east-1:ACCOUNT_ID:cluster/soat-oficina-eks
```

The script waits for the Secrets Store CSI CRD and applies only hml/prod namespaces
and namespace-scoped roles/bindings for SecretProviderClass. Each deploy role joins
only its own `oficina-app-deploy-hml` or `oficina-app-deploy-prod` group. No app role
receives cluster-admin. The Helm chart must not create/adopt these namespaces.
Keep this bootstrap separate from the AWS Terraform provider lifecycle so cluster
teardown does not depend on a reachable Kubernetes provider.
